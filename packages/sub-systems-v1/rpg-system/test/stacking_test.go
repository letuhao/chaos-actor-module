...
func TestStackingEngine_ApplyModifiers(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: 158.4, // (100 + 20) * 1.1 * 1.2 = 120 * 1.1 * 1.2 = 158.4
		},
	}
...
func TestStackingEngine_ApplyStackLimits(t *testing.T) {
	...
	if result[0].Priority != 4 || result[1].Priority != 3 {
		t.Errorf("Expected highest priority modifiers to be kept, got priorities %d and %d", result[0].Priority, result[1].Priority)
	}
}
